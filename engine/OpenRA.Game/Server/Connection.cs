...
namespace OpenRA.Server
{
	public sealed class Connection : IDisposable
	{
		...
		readonly InMemoryLink inMemoryLink;
		bool sentHandshakePreamble;
		...
		public Connection(Server server, InMemoryLink link, string authToken)
		{
			PlayerIndex = server.ChooseFreePlayerIndex();
			AuthToken = authToken;
		...
		{
			if (inMemoryLink != null)
			{
				if (inMemoryLink.ClientClosed || inMemoryLink.ServerClosed)
					return false;

				if (!sentHandshakePreamble)
				{
					sentHandshakePreamble = true;
					inMemoryLink.ServerToClient.Enqueue(data);
					return true;
				}

				if (data.Length < 8 || BitConverter.ToInt32(data, 0) != data.Length - 8)
					throw new InvalidDataException("Invalid server frame.");

				// NetworkConnectionReceive reads the length separately, then retains the client index
				// and the frame-prefixed payload. Store that same byte sequence for the in-memory client.
				inMemoryLink.ServerToClient.Enqueue(data[4..]);
				return true;
			}
		...
		internal void PumpInMemory(Server server)
		{
			if (inMemoryLink == null || disconnectQueued)
				return;

			while (inMemoryLink.ClientToServer.TryDequeue(out var packet))
			{
				lastReceivedTime = Game.RunTime;
				TimeoutMessageShown = false;

				if (server.IsMultiplayer && packet.Data.Length > MaxOrderLength)
				{
					Log.Write("server", $"Closing in-memory connection because of excessive order length: {packet.Data.Length}");
					inMemoryLink.ClientClosed = true;
					break;
				}

				server.OnConnectionPacket(this, packet.Frame, packet.Data);
			}

			// A live in-memory peer is pumped by the game loop, so it cannot time out while the tab is active.
			if (!inMemoryLink.ClientClosed)
			{
				lastReceivedTime = Game.RunTime;
				TimeoutMessageShown = false;
			}
		...
		public void Dispose()
		{
			if (inMemoryLink != null)
			{
				inMemoryLink.ServerClosed = true;
				return;
			}
