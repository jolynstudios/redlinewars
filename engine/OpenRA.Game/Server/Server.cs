...
namespace OpenRA.Server
{
	...
	public sealed class Server
	{
		...
		readonly bool runCooperatively;
		...
		public void Shutdown()
		{
			...
			if (runCooperatively && !stepping)
				Step(0);
		}

		public void EndGame()
		{
			if (Interlocked.Exchange(ref gameEnded, 1) != 0)
				return;
		...
		public Server(ServerSettings settings, ModData modData, ServerType type, int? randomSeed = null)
			: this([], settings, modData, type, randomSeed) { }

		public Server(List<IPEndPoint> endpoints, ServerSettings settings, ModData modData, ServerType type, int? randomSeed = null)
		{
			Log.AddChannel("server", "server.log", true);
			runCooperatively = endpoints.Count == 0;
			...
			if (!runCooperatively && listeners.Count == 0)
				throw lastException;
			...
			this.randomSeed = randomSeed ?? (int)DateTime.Now.ToBinary();
			...
			if (runCooperatively)
				...
				new Thread(ServerThread) { IsBackground = true, Name = "ServerThread" }.Start();
		}
		...
		{
			StartServer();
			while (RunServerIteration(1, 1000)) { }
		}

		bool RunServerIteration(int maxEvents, int firstEventWaitMilliseconds)
		{
			if (serverStopped)
				return false;
			...
			if (State != ServerState.ShuttingDown)
			{
				...
				{
					var waitMilliseconds = i == 0 ? firstEventWaitMilliseconds : 0;
					if (!events.TryTake(out var e, waitMilliseconds))
						break;
		...
		public void Step(int maxEvents = 64)
		{
			if (!runCooperatively || serverStopped || stepping)
				return;
			...
			try
			{
				foreach (var connection in Conns.ToArray())
					connection.PumpInMemory(this);

				RunServerIteration(maxEvents, 0);
			}
			finally
			{
				stepping = false;
		...
		void AcceptConnection(Socket socket)
		{
			...
			AcceptConnection(new Connection(this, socket, token));
		}

		public InMemoryLink ConnectInMemoryClient()
		{
			if (!runCooperatively)
				throw new InvalidOperationException("In-memory clients can only connect to a cooperative server.");

			if (State != ServerState.WaitingPlayers)
				throw new InvalidOperationException("Clients can only connect while the server is waiting for players.");

			// Validate player identity by asking them to sign a random blob of data
			// which we can then verify against the player public key database
			var token = Convert.ToBase64String(OpenRA.Exts.MakeArray(256, _ => (byte)Random.Next()));
			var link = new InMemoryLink();
			AcceptConnection(new Connection(this, link, token));
			return link;
		}

		void AcceptConnection(Connection newConn)
		{
			try
			{
			...
			lock (LobbyInfo)
			{
				...
				if (pingUpdated.ElapsedMilliseconds > 5000)
				{
					...
					foreach (var c in LobbyInfo.Clients)
					{
						...
						nodes.Add(new MiniYamlNode($"Ping@{c.Index}", FieldSaver.FormatValue(c.PingMs)));
					}
