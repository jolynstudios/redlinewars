...
namespace OpenRA
{
	...
	public class DebugSettings : SettingsModule
	{
		...
		[Desc("Run local servers in-process without sockets. Always enabled on single-threaded runtimes.")]
		public bool InProcessServer = false;

		[Desc("Override the random seed for local servers. Empty uses a time-derived seed.")]
		public int? ServerRandomSeed = null;
