...
namespace OpenRA
{
	...
	public static class Game
	{
		...
		// Local servers run in-process without sockets on single-threaded runtimes
		// (browser-wasm), or on demand for testing via Debug.InProcessServer.
		// The cooperative server is pumped from LogicTick instead of its own thread.
		static Server.Server inProcessServer;

		static bool UseInProcessServer => !Platform.SupportsThreads || Settings.Debug.InProcessServer;
		...
		// Hosts (browser-wasm) can substitute their own platform implementation
		// instead of side-loading OpenRA.Platforms.*.dll via the assembly loader.
		public static Func<string, IPlatform> PlatformFactory;
		public static Func<ConnectionTarget, NetworkConnection> ConnectionFactory;
		public static string HandshakeModOverride;
		public static string HandshakeVersionOverride;

		public static OrderManager JoinServer(ConnectionTarget endpoint, string password, bool recordReplay = true)
		{
			var newConnection = inProcessServer != null && inProcessServer.State == Server.ServerState.WaitingPlayers
				? new InMemoryNetworkConnection(inProcessServer.ConnectInMemoryClient())
				: ConnectionFactory != null ? ConnectionFactory(endpoint) : new TcpNetworkConnection(endpoint);
			if (recordReplay)
		...
		internal static void StartGame(Map map, WorldType type)
		{
			...
			// Configuring LOH compaction is not supported on browser-wasm.
		...
		// Hosts that cannot run a blocking loop (browser-wasm) initialize here and
		// drive the returned stepper from their own scheduler (e.g. requestAnimationFrame).
		public static GameStepper InitializeHosted(string[] args)
		{
			Initialize(new Arguments(args));
			GC.Collect();
			...
			return new GameStepper(RunTime);
		}
		...
		internal static void LogicTick()
		{
			PerformDelayedActions();

			// Cooperative servers have no thread of their own; drive them from the game loop.
			inProcessServer?.Step();
		...
		internal static void RenderTick()
		{
		...
		static void Loop()
		{
			...
			// ---
			// The scheduling rules live in GameStepper so that browser hosts
			// can drive the same logic from requestAnimationFrame.
			// ---
			...
			var stepper = new GameStepper(RunTime);
			while (State == RunStatus.Running)
			{
				var wait = stepper.Step(RunTime);
				if (wait > 0)
		...
		public static ConnectionTarget CreateLocalServer(string map, bool isSkirmish = false)
		{
			...
			{
				server = inProcessServer = new Server.Server(
					settings, ModData, isSkirmish ? ServerType.Skirmish : ServerType.Local, Settings.Debug.ServerRandomSeed);
				return null;
			}
			...
			server = new Server.Server(
				endpoints, settings, ModData, isSkirmish ? ServerType.Skirmish : ServerType.Local, Settings.Debug.ServerRandomSeed);
