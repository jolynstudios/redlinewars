...
namespace OpenRA
{
	...
	{
		IPresentationObserver presentationObserver;
		...
		/// <summary>
		/// Attach an unsynced presentation observer. This is deliberately not a property:
		/// simulation code may publish values through the Notify methods below, but has no
		/// observer value to inspect and no result to consume.
		/// </summary>
		public void AttachPresentationObserver(IPresentationObserver observer)
			=> presentationObserver = observer;

		/// <summary>Detach only the observer that owns the current attachment.</summary>
		public void DetachPresentationObserver(IPresentationObserver observer)
		{
			if (ReferenceEquals(presentationObserver, observer))
				presentationObserver = null;
		}

		public void NotifyWeaponFired(in PresentationWeaponFire value)
		{
			var observer = presentationObserver;
			if (observer == null)
				return;

			try { observer.WeaponFired(value); }
			catch { DisablePresentationObserver(observer); }
		}

		public void NotifyProjectileImpacted(in PresentationProjectileImpact value)
		{
			var observer = presentationObserver;
			if (observer == null)
				return;

			try { observer.ProjectileImpacted(value); }
			catch { DisablePresentationObserver(observer); }
		}

		public void NotifyActorDamaged(in PresentationActorDamage value)
		{
			var observer = presentationObserver;
			if (observer == null)
				return;

			try { observer.ActorDamaged(value); }
			catch { DisablePresentationObserver(observer); }
		}

		public void NotifyActorDestroyed(in PresentationActorDestroyed value)
		{
			var observer = presentationObserver;
			if (observer == null)
				return;

			try { observer.ActorDestroyed(value); }
			catch { DisablePresentationObserver(observer); }
		}

		public void NotifyProductionCompleted(in PresentationProductionComplete value)
		{
			var observer = presentationObserver;
			if (observer == null)
				return;

			try { observer.ProductionCompleted(value); }
			catch { DisablePresentationObserver(observer); }
		}

		public void NotifyStructureBuilt(in PresentationStructureBuilt value)
		{
			var observer = presentationObserver;
			if (observer == null)
				return;

			try { observer.StructureBuilt(value); }
			catch { DisablePresentationObserver(observer); }
		}

		void DisablePresentationObserver(IPresentationObserver observer)
		{
			if (ReferenceEquals(presentationObserver, observer))
				presentationObserver = null;
		}
