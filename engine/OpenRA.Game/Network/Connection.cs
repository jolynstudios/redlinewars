...
namespace OpenRA.Network
{
	...
	public abstract class NetworkConnection : IConnection
	{
		...
		protected NetworkConnection(ConnectionTarget target)
		{
			Target = target;
		}
		...
		protected void CompleteHandshake(int handshakeProtocol, int localClientId)
		{
		...
		protected void EnqueueReceivedPacket(int fromClient, byte[] data)
		{
			receivedPackets.Enqueue((fromClient, data));
		}

		protected virtual void PumpTransport() { }

		protected abstract void SendBytes(MemoryStream ms);
	...
	public sealed class TcpNetworkConnection : NetworkConnection
	{
		TcpClient tcp;

		public TcpNetworkConnection(ConnectionTarget target)
			: base(target)
		{
			new Thread(NetworkConnectionConnect)
			{
				Name = $"{nameof(NetworkConnection)} (connect to {target})",
				IsBackground = true
			}.Start();
		}
		...
		void NetworkConnectionConnect()
		{
			...
			foreach (var endpoint in Target.GetConnectEndPoints())
			{
				...
				{
					Name = $"{nameof(NetworkConnection)} (connect to {endpoint})",
					IsBackground = true
				}.Start();
			}
			...
			else if (queue.TryTake(out tcp, 5000))
			{
				...
				{
					Name = $"{nameof(NetworkConnection)} (receive from {tcp.Client.RemoteEndPoint})",
					IsBackground = true
				}.Start();
			}
			else
		...
		void NetworkConnectionReceive()
		{
			try
			{
				...
				CompleteHandshake(handshakeProtocol, stream.ReadInt32());
				...
				while (true)
				{
					...
					EnqueueReceivedPacket(client, buf);
				}
			}
			catch (Exception ex)
			{
	...
	public sealed class InMemoryNetworkConnection : NetworkConnection
	{
		readonly InMemoryLink link;
		bool handshakeComplete;

		public InMemoryNetworkConnection(InMemoryLink link)
			: base(new ConnectionTarget("127.0.0.1", 0))
		{
			this.link = link;
		...
		protected override void PumpTransport()
		{
			try
			{
				while (link.ServerToClient.TryDequeue(out var chunk))
				{
					if (!handshakeComplete)
					{
						if (chunk.Length != 8)
							throw new InvalidDataException($"Invalid handshake preamble length {chunk.Length}.");

						CompleteHandshake(BitConverter.ToInt32(chunk, 0), BitConverter.ToInt32(chunk, 4));
						handshakeComplete = true;
						continue;
					}

					if (chunk.Length < 8)
						throw new InvalidDataException($"Invalid server frame length {chunk.Length}.");

					var fromClient = BitConverter.ToInt32(chunk, 0);
					EnqueueReceivedPacket(fromClient, chunk[4..]);
				}
			}
			catch (Exception ex)
			{
				ErrorMessage = "Connection failed";
				Log.Write("client", $"In-memory connection failed: {ex.Message}");
				link.ClientClosed = true;
				SetConnectionState(ConnectionState.NotConnected);
			}

			if (link.ServerClosed)
				SetConnectionState(ConnectionState.NotConnected);
		}

		protected override void SendBytes(MemoryStream ms)
		{
			ObjectDisposedException.ThrowIf(link.ClientClosed || link.ServerClosed, link);

			ms.Position = 0;
			while (ms.Position < ms.Length)
			{
				if (ms.Length - ms.Position < 8)
					throw new InvalidDataException("Invalid client frame header.");

				var length = ms.ReadInt32();
				if (length < 4 || length > ms.Length - ms.Position)
					throw new InvalidDataException($"Invalid client frame length {length}.");

				var frame = ms.ReadInt32();
				link.ClientToServer.Enqueue((frame, ms.ReadBytes(length - 4)));
			}
		}
