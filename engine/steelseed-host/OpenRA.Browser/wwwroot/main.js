...
const useWorker = mode === 'game' && params.get('worker') !== '0' && typeof Worker !== 'undefined';
if (useWorker) {
	try {
		const { createPageBridge } = await import('./sim-proxy.js');
		const worker = new Worker(new URL('./sim-worker.js', import.meta.url), { type: 'module' });
		const { bridge, ora } = createPageBridge(worker);
		...
		for (const [k, v] of params.entries()) {
			if (k !== 'mode' && k !== 'platform' && k !== 'glprobe' && k !== 'worker')
				argv.push(`${k}=${v}`);
		}
		const ready = new Promise((resolve, reject) => {
			const onMessage = event => {
				const message = event.data;
				if (message?.type === 'ready') { worker.removeEventListener('message', onMessage); resolve(); }
				if (message?.type === 'failed') { worker.removeEventListener('message', onMessage); reject(new Error(message.error ?? 'simulation worker failed')); }
			};
			worker.addEventListener('message', onMessage);
		});
		worker.postMessage({ type: 'init', query: location.search, argv });
		await ready;
		globalThis.steelseedBridge = bridge;
		globalThis.ora = ora;
		bridgeReadiness?.resolve(bridge);
		document.body.classList.add('game-running');
		status.textContent = 'Game running';
		print('[js] simulation worker started');
		globalThis.__s1_done = { exitCode: 0, worker: true };
	} catch (err) {
	...
	if (mode !== 'game') {
		...
		globalThis.ora = P;
		// The managed connection only queues transport work. The browser owns the
		// native WebSocket and must drain that queue on the same pump that advances
		// Frame(); without this wiring every join remains at netframe 0 forever.
		const mpSocket = installMpSocket(P, localHeapViewU8);
		const steelseedBridge = createSteelseedBridge(P, localHeapViewU8, mpSocket);
		...
		const flushStorage = () => {
			try {
				// The rule-8 host intentionally has no BrowserStorage export. Storage
				// persistence is an optional legacy-host capability and must never stop
				// the simulation pump when the shared page host is paired with it.
