	...
	sealed class Program
	{
		...
		static void Run(string[] args)
		{
			...
			while (true)
			{
				...
				// Hosted rooms (roomhost.mjs) set REDLINE_BIND=loopback: players arrive only through the
				// node's WS mux, so the raw game port must not be reachable from other machines, where it
				// would bypass the host key, Ranked claims and relay limits.
				var loopbackOnly = string.Equals(Environment.GetEnvironmentVariable("REDLINE_BIND"), "loopback", StringComparison.OrdinalIgnoreCase);
				var endpoints = loopbackOnly
					? new List<IPEndPoint> { new(IPAddress.IPv6Loopback, settings.ListenPort), new(IPAddress.Loopback, settings.ListenPort) }
					: new List<IPEndPoint> { new(IPAddress.IPv6Any, settings.ListenPort), new(IPAddress.Any, settings.ListenPort) };
				...
				while (true)
				{
					...
					if (server.State == ServerState.GameStarted && server.Conns.Count < 1)
					{
						...
						// Finalize the replay synchronously before the wrapper disposes the
						// mod and starts its next instance. Shutdown is serviced by the
						// server thread; without this call roomhost can only custody a
						// metadata-less .orarep that ReplayMetadata.Read rejects.
						server.EndGame();
						server.Shutdown();
						break;
					}
