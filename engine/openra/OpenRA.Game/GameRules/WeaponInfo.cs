...
namespace OpenRA.GameRules
{
	public class ProjectileArgs
	{
		...
		// Observer metadata only: never read by projectile motion, damage, or the sync hash.
		public ushort PresentationArmament = ushort.MaxValue;
		public ushort PresentationBarrel = ushort.MaxValue;
		public uint PresentationShot;
	...
	/// <summary>Value-only observer for actual weapon impacts; no simulation authority.</summary>
	public interface INotifyWeaponImpact
	{
		void ObserveImpact(WeaponInfo weapon, WPos position, WVec direction, Actor source, ushort armament, ushort barrel, uint shot);
	}
	...
	public class WarheadArgs
	{
		...
		{
			Weapon = args.Weapon;
			PresentationArmament = args.PresentationArmament;
			PresentationBarrel = args.PresentationBarrel;
			PresentationShot = args.PresentationShot;
	...
	public sealed class WeaponInfo
	{
		...
		public void Impact(in Target target, WarheadArgs args)
		{
			...
			// Only projectiles fill ImpactPosition. Projectile-less impacts (NukeLaunch, death and
			// crate explosions) land their warheads on the target, so report that position rather
			// than the world origin, where presentation filtering silently dropped them.
			var impactPosition = args.ImpactPosition == WPos.Zero ? target.CenterPosition : args.ImpactPosition;
			var direction = args.Source.HasValue ? args.Source.Value - impactPosition : new WVec(0, 0, -1024);
			foreach (var observer in world.WorldActor.TraitsImplementing<INotifyWeaponImpact>())
				observer.ObserveImpact(this, impactPosition, direction, args.SourceActor,
					args.PresentationArmament, args.PresentationBarrel, args.PresentationShot);
