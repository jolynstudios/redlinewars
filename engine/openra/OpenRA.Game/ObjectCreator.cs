...
namespace OpenRA
{
	public sealed class ObjectCreator : IDisposable
	{
		...
		// Assemblies provided by the host ahead of time, keyed by manifest file name.
		// Hosts that cannot load assemblies from disk (e.g. browser-wasm) register
		// their statically referenced mod assemblies here before creating a ModData.
		static readonly Dictionary<string, Assembly> PreRegisteredAssemblies = [];

		public static void RegisterAssembly(Assembly assembly)
		{
			PreRegisteredAssemblies[assembly.GetName().Name + ".dll"] = assembly;
		}
