...
namespace OpenRA.Mods.Cnc.Projectiles
{
	...
	public class TeslaZap : IProjectile, ISync, IProjectileFlight
	{
		...
		// STEELSEED presentation surface. Getters over state this projectile already keeps;
		// nothing here is read by the simulation or by the sync hash. See IProjectileFlight.
		// A zap does not travel. It exists between the coil's muzzle and whatever it is holding,
		// for Duration ticks, and it re-reads that target every tick while TrackTarget is set.
		WPos IProjectileFlight.FlightPosition => args.Source;
		WVec IProjectileFlight.FlightVelocity => WVec.Zero;
		WPos IProjectileFlight.FlightTarget => target;
		int IProjectileFlight.FlightRemainingTicks => ticksUntilRemove > 0 ? ticksUntilRemove : 0;
		bool IProjectileFlight.FlightIsBeam => true;
		WeaponInfo IProjectileFlight.FlightWeapon => args.Weapon;
		uint IProjectileFlight.FlightSourceActorId => args.SourceActor?.ActorID ?? 0;
		WPos IProjectileFlight.FlightLaunchPosition => args.Source;
		ushort IProjectileFlight.FlightArmament => args.PresentationArmament;
		ushort IProjectileFlight.FlightBarrel => args.PresentationBarrel;
		uint IProjectileFlight.FlightShot => args.PresentationShot;
