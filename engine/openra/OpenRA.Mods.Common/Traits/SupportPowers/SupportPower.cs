...
namespace OpenRA.Mods.Common.Traits
{
	...
	public abstract class SupportPower : PausableConditionalTrait<SupportPowerInfo>
	{
		...
		{
			// The manager's RadarPings is a Lazy around TraitOrDefault: the LAZY is never
			// null, but its VALUE is whenever the world rules ship without the RadarPings
			// trait (the assetless generated rules strip presentation-only world traits).
			// Testing the lazy instead of the value crashed the whole simulation with a
			// NullReferenceException the moment a radar-pinging support power activated
			// (Nuke, Iron Curtain, Chronoshift), whether a bot or a player fired it.
			var radarPings = manager.RadarPings?.Value;
