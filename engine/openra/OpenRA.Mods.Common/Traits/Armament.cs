...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class Armament : PausableConditionalTrait<ArmamentInfo>, ITick
	{
		...
		// Only the presentation observer reads these; deliberately not [Sync].
		public uint PresentationShot { get; private set; }
		public WPos PresentationSource { get; private set; }
		Turreted turret;
		Hovers hovers;
		...
		protected virtual void FireBarrel(Actor self, IFacing facing, in Target target, Barrel barrel)
		{
			...
			{
				Weapon = Weapon,
				PresentationArmament = (ushort)self.TraitsImplementing<Armament>().TakeWhile(a => !ReferenceEquals(a, this)).Count(),
				PresentationBarrel = (ushort)Array.IndexOf(Barrels, barrel),
				Facing = MuzzleFacing(),
			...
			{
				args.PresentationShot = ++PresentationShot;
				PresentationSource = args.Source;
