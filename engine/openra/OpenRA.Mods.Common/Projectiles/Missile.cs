...
namespace OpenRA.Mods.Common.Projectiles
{
	...
	public class Missile : IProjectile, ISync, IProjectileFlight
	{
		enum States
		{
			Freefall,
		...
		WVec IProjectileFlight.FlightVelocity => velocity;
		WPos IProjectileFlight.FlightTarget => targetPosition;

		// A missile homes: its remaining flight is a function of a target that is still moving,
		// so there is no honest tick count to publish. -1 says 'unknown' instead of inventing one.
		int IProjectileFlight.FlightRemainingTicks => -1;
