...
{
	public class NukeLaunch : IProjectile, ISpatiallyPartitionable, IProjectileFlight
	{
		...
		// STEELSEED: presentation-only read surface, so the browser draws the real missile climbing
		// out of the silo and falling on its target. Getters over state the missile already keeps;
		// nothing here is read by the simulation or by the sync hash. See IProjectileFlight.
		WPos IProjectileFlight.FlightPosition => pos;
		WVec IProjectileFlight.FlightVelocity => ticks < turn
			? (ascendTarget - ascendSource) / System.Math.Max(1, turn)
			: (descendTarget - descendSource) / System.Math.Max(1, impactDelay - turn);
		WPos IProjectileFlight.FlightTarget => descendTarget;
		int IProjectileFlight.FlightRemainingTicks => System.Math.Max(0, impactDelay - ticks);
		bool IProjectileFlight.FlightIsBeam => false;
		WeaponInfo IProjectileFlight.FlightWeapon => weapon;
		uint IProjectileFlight.FlightSourceActorId => 0;
		WPos IProjectileFlight.FlightLaunchPosition => ascendSource;
		ushort IProjectileFlight.FlightArmament => ushort.MaxValue;
		ushort IProjectileFlight.FlightBarrel => ushort.MaxValue;
		uint IProjectileFlight.FlightShot => 0;
		bool IProjectileFlight.FlightLaunched => isLaunched && !detonated;
	}
}
