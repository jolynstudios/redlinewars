...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class Armament : PausableConditionalTrait<ArmamentInfo>, ITick
	{
		...
		readonly ushort presentationWeaponClass;
		readonly ushort presentationCaliber;
		ushort presentationArmament;
			...
			: base(info)
		{
			...
			presentationWeaponClass = StableId(info.Weapon);
			foreach (var warhead in Weapon.Warheads)
			{
				if (warhead is DamageWarhead damage)
				{
					presentationCaliber = (ushort)Math.Clamp(Math.Abs((long)damage.Damage), 0, ushort.MaxValue);
					break;
				}
			}
		...
		{
			ushort armamentIndex = 0;
			foreach (var armament in self.TraitsImplementing<Armament>())
			{
				if (ReferenceEquals(armament, this))
				{
					presentationArmament = armamentIndex;
					break;
				}
		...
		protected virtual void FireBarrel(Actor self, IFacing facing, in Target target, Barrel barrel)
		{
			...
			var args = new ProjectileArgs
			{
				...
				GuidedTarget = target,
				PresentationWeaponClass = presentationWeaponClass,
				PresentationCaliber = presentationCaliber,
			};
			...
			ScheduleDelayedAction(Info.FireDelay, Burst, (burst) =>
			{
				if (args.Weapon.Projectile != null)
				{
					...
					Recoil = Info.Recoil;
					self.World.NotifyWeaponFired(new PresentationWeaponFire(
						self.ActorID,
						presentationArmament,
						args.Source,
						args.Facing,
						args.PresentationWeaponClass,
						args.PresentationCaliber,
						Info.Weapon));
				}
		...
		static ushort StableId(string value)
		{
			...
			return (ushort)(hash % byte.MaxValue + 1);
		}
