...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ProductionQueue : IResolveOrder, ITick, ITechTreeElement, INotifyOwnerChanged, INotifyKilled, INotifySold, ISync, INotifyTransform, INotifyCreated
	{
		...
		void CacheProducibles()
		{
			...
			// Presentation reads this by index from the unsynced snapshot observer. Cache and
			// sort only when the tech-tree catalogue is rebuilt; enumerating the LINQ-backed
			// AllItems() view during every snapshot would allocate inside the sim tick.
			presentationItems = Producible.Keys.ToArray();
			Array.Sort(presentationItems, static (a, b) =>
			{
				var ao = a.TraitInfo<BuildableInfo>().BuildPaletteOrder;
				var bo = b.TraitInfo<BuildableInfo>().BuildPaletteOrder;
				var order = ao.CompareTo(bo);
				return order != 0 ? order : string.CompareOrdinal(a.Name, b.Name);
			});
		}

		/// <summary>
		/// Allocation-free, read-only production catalogue for presentation observers.
		/// These methods expose values only; they neither resolve orders nor mutate the queue.
		/// </summary>
		public int PresentationItemCount => presentationItems.Length;

		public ActorInfo PresentationItemAt(int index) => presentationItems[index];

		public bool PresentationItemVisible(ActorInfo actor)
			=> Enabled && Producible.TryGetValue(actor, out var state) && (state.Visible || state.Buildable);

		public bool PresentationItemBuildable(ActorInfo actor)
		{
			if (!Enabled || !Producible.TryGetValue(actor, out var state))
				return false;

			var anyProduction = productionTraits.Length == 0;
			for (var i = 0; i < productionTraits.Length; i++)
				anyProduction |= !productionTraits[i].IsTraitDisabled;

			if (!anyProduction || !(state.Buildable || developerMode.AllTech))
				return false;

			if (Info.PayUpFront && GetProductionCost(actor) > playerResources.GetCashAndResources() &&
				PresentationQueuedCountFor(actor) == 0)
				return false;

			return true;
		}

		public int PresentationQueuedCount => Queue.Count;

		public int PresentationQueuedCountFor(ActorInfo actor)
		{
			var count = 0;
			for (var i = 0; i < Queue.Count; i++)
				if (Queue[i].Item == actor.Name)
					count++;
		...
		{
			// This is read by the browser presentation observer on every emitted tick.
			// Index the concrete list directly so this hot path cannot fall through an
			// IEnumerable/LINQ implementation that boxes an enumerator under AOT.
			return Queue.Count == 0 ? null : Queue[0];
		}

		/// <summary>
		/// Producer chosen during the queue's simulation tick for allocation-free presentation
		/// reads. Queue implementations whose producer is not cached retain the live fallback.
		/// </summary>
		public virtual Actor PresentationMostLikelyProducerActor => MostLikelyProducer().Actor;
		...
		{
			// Keep the stable OrderBy(IsTraitPaused).FirstOrDefault semantics without allocating
			// an ordered LINQ iterator. SnapshotEmitter reads this once per production structure
			// per emitted tick, so the iterator was an observable hot-path allocation.
			Production trait = null;
			for (var i = 0; i < productionTraits.Length; i++)
			{
				var candidate = productionTraits[i];
				if (candidate.IsTraitDisabled || !candidate.Info.Produces.Contains(Info.Type))
					continue;

				if (trait == null || (trait.IsTraitPaused && !candidate.IsTraitPaused))
					trait = candidate;

				if (!trait.IsTraitPaused)
					break;
			}
