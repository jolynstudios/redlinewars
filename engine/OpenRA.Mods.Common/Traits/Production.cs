	...
	public class Production : PausableConditionalTrait<ProductionInfo>, INotifyOwnerChanged
	{
		...
		public virtual void DoProduction(Actor self, ActorInfo producee, ExitInfo exitinfo, string productionType, TypeDictionary inits)
		{
			...
			self.World.AddFrameEndTask(w =>
			{
				...
				self.World.NotifyProductionCompleted(new PresentationProductionComplete(
					(byte)Math.Clamp(self.Owner.ClientIndex, 0, byte.MaxValue),
					StableQueueId(productionType),
					newUnit.ActorID));
				if (!self.IsDead)
		...
		/// <summary>
		/// Stable presentation id for a production category. Snapshot queue summaries and
		/// production-complete events must use this single function or the UI cannot join them.
		/// </summary>
		public static byte StableQueueId(string value)
		{
			...
			return (byte)(hash % byte.MaxValue + 1);
		}
