...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class Health : IHealth, ISync, ITick, INotifyCreated, INotifyOwnerChanged
	{
		...
		public void InflictDamage(Actor self, Actor attacker, Damage damage, bool ignoreModifiers)
		{
			...
			var actualDamage = oldHp - HP;
			if (actualDamage > 0)
			{
				self.World.NotifyActorDamaged(new PresentationActorDamage(
					self.ActorID,
					(byte)(255L * HP / MaxHP),
					0,
					attacker?.ActorID ?? 0));
			...
			if (HP == 0)
			{
				self.World.NotifyActorDestroyed(new PresentationActorDestroyed(
					self.ActorID,
					self.CenterPosition,
					0,
					(byte)Math.Clamp(255L * actualDamage / MaxHP, 0, 255)));
