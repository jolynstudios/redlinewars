...
try {
	...
	// stock Doubles map (126 Neutral actors, no staged e7/powr). worker=0 pins the
	...
	// Measured on the recomposed engine: the first bot decision tick stalls the sim for
	// ~12 s (one Frame() call, ticks 9→11, with or without any staged order; the session
	// health monitor logs it as a transient halt→resume). Issuing C4 inside that window
	// starves the 16 s observation deadline, so hold the order until the sim is steady;
	// from tick 20 the demolition itself takes ~3.5 s (Riki enters, plants, exits).
	await page.waitForFunction(() => steelseed.ctx.snapshot.tick >= 20, undefined, { timeout: 60000, polling: 100 })
	const initial = await page.evaluate(() => {
		...
		return { riki: row('e7'), target: row('powr') }
	})
	assert.ok(initial.riki && initial.target, 'fixture has Riki and a demolishable enemy building')
